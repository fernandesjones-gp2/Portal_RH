...
export default function ConfiguracoesPage() {
  ...
  const handleToggleModulePermission = (path, action) => {
    ...
    if ((action === 'create' || action === 'edit' || action === 'delete') && currentPerms[path][action]) currentPerms[path].view = true;
    if (action === 'view' && !currentPerms[path].view) currentPerms[path] = { view: false, create: false, edit: false, delete: false };
  ...
  const openUserAccessModal = (user) => { setEditingUser({ ...user, unit_ids: getSafeUserUnits(user) }); setIsUserAcessModalOpen(true); };

  const toggleUserUnit = (unitId) => {
    const currentUnits = [...(editingUser.unit_ids || [])];
    if (currentUnits.includes(unitId)) setEditingUser({ ...editingUser, unit_ids: currentUnits.filter(id => id !== unitId) });
    else setEditingUser({ ...editingUser, unit_ids: [...currentUnits, unitId] });
  };
  ...
  const handleSaveUserAccess = async (e) => {
    ...
    try {
      ...
      await api.users.update(editingUser.id, { 
        role: editingUser.role, unit_ids: finalUnitIds, unit_id: finalUnitIds.length > 0 ? finalUnitIds[0] : null,
      ...
      setIsUserAcessModalOpen(false); fetchAllData();
  ...
  async function handleAddRole(e) { 
    ...
    try { 
      await api.jobRoles.create({ name: newRoleForm.name.toUpperCase(), sector: newRoleForm.sector.toUpperCase() }); 
      setNewRoleForm({ name: '', sector: '' }); 
      fetchAllData(); 
    } catch (error) {} 
  }
  ...
  async function handleUpdateRole() { 
    ...
    try { 
      await api.jobRoles.update(editingRole.id, { name: editingRole.name.toUpperCase(), sector: editingRole.sector ? editingRole.sector.toUpperCase() : '' }); 
      setEditingRole(null); 
      fetchAllData(); 
    } catch (error) {} 
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem', paddingBottom: '3rem' }}>
      ...
      <div className="glass-panel" style={{ padding: '2rem', backgroundColor: 'var(--surface-color)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          ...
          <button onClick={() => { setEditingCustomRole(null); setRoleForm({ name: '', permissions: {} }); setIsRoleModalOpen(true); }} className="btn-primary" style={{ padding: '0.5rem 1rem', fontSize: '0.85rem' }}>
          ...
          {customRoles.map(role => (
            <div key={role.id} style={{ backgroundColor: 'var(--bg-color)', padding: '1.25rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                {role.name !== 'ADMIN' && ( <button onClick={() => handleDeleteCustomRole(role.id)} className="btn-secondary" style={{ padding: '0.4rem', color: 'var(--danger-color)' }}><Trash2 size={14} /></button> )}
              </div>
            </div>
          ))}
        </div>
              ...
              {usersList.map(user => {
                ...
                return (
                  <tr key={user.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                    <td style={{ padding: '0.75rem' }}><div style={{ fontWeight: '600', color: 'var(--text-main)' }}>{user.name || 'Sem nome'}</div><div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{user.email}</div></td>
                    ...
                    <td style={{ padding: '0.75rem' }}><span style={{ fontWeight: 'bold', fontSize: '0.85rem' }}>{user.role || 'Sem Perfil'}</span></td>
                    <td style={{ padding: '0.75rem' }}><span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{safeUnits.length > 0 ? `${safeUnits.length} Unidade(s)` : 'Acesso Geral (Todas)'}</span></td>
                    <td style={{ padding: '0.75rem', textAlign: 'right', display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }}>
                      <button onClick={() => openUserAccessModal(user)} className="btn-secondary" style={{ padding: '0.3rem 0.6rem', fontSize: '0.75rem', fontWeight: 'bold', borderColor: 'var(--saritur-orange)', color: 'var(--saritur-orange)' }}><UserCog size={14} style={{ marginRight: '4px' }}/> Configurar</button>
      ...
      <div className="glass-panel" style={{ padding: '2rem', backgroundColor: 'var(--surface-color)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          ...
          <button onClick={() => { setEditingWidget(null); setWidgetForm({ title: '', chart_type: 'kpi', metric_type: 'count', status_filter: 'Todos', color: '#F37137', roles_visible: customRoles.map(r=>r.name), advanced_config: { format: 'integer', size: 'half', groupBy: 'all', funnelColors: defaultFunnelColors }, visibility_type: 'generic' }); setIsWidgetModalOpen(true); }} className="btn-primary" style={{ padding: '0.5rem 1rem', fontSize: '0.85rem' }}><Plus size={16} /> Novo Indicador</button>
        </div>
          ...
          {widgets.map(w => (
            <div key={w.id} style={{ backgroundColor: 'var(--bg-color)', padding: '1rem', borderRadius: 'var(--radius-md)', borderLeft: `4px solid ${w.color}`, border: '1px solid var(--border-color)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div><h3 style={{ fontWeight: 'bold', fontSize: '1rem', color: 'var(--text-main)' }}>{w.title}</h3><p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{w.chart_type.toUpperCase()} | {w.metric_type}</p></div>
                <div style={{ display: 'flex', gap: '0.5rem' }}><button onClick={() => { setEditingWidget(w); const safeConfig = w.advanced_config || { format: 'integer', size: 'half', groupBy: 'all', funnelColors: defaultFunnelColors }; const safeRoles = parseSafeArray(w.roles_visible); setWidgetForm({...w, roles_visible: safeRoles, advanced_config: safeConfig, visibility_type: safeRoles.length > 4 ? 'generic' : 'internal'}); setIsWidgetModalOpen(true); }}><Edit2 size={14} color="var(--text-muted)"/></button><button onClick={() => handleDeleteWidget(w.id)}><Trash2 size={14} color="var(--danger-color)"/></button></div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="glass-panel" style={{ padding: '2rem', backgroundColor: 'var(--surface-color)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
          ...
          <div style={{ borderRight: '1px solid var(--border-color)', paddingRight: '1rem' }}>
            <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '600', marginBottom: '0.5rem' }}>Funções e Setores</label>
            <select style={{ width: '100%', marginBottom: '1rem', padding: '0.5rem' }} value={selectedRoleId} onChange={e => { setSelectedRoleId(e.target.value); setEditingRole(null); }}>
              <option value="">-- Selecione --</option>
              {roles.map(r => <option key={r.id} value={r.id}>{r.name} {r.sector ? `(${r.sector})` : ''}</option>)}
            </select>
            
            {selectedRoleId && currentRoleObj && (
              <div style={{ backgroundColor: 'var(--bg-color)', padding: '0.75rem', borderRadius: 'var(--radius-md)', display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  {editingRole ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', width: '80%' }}>
                      <input placeholder="Cargo..." style={{padding: '0.3rem', fontSize: '0.8rem'}} value={editingRole.name} onChange={e => setEditingRole({...editingRole, name: e.target.value})} />
                      <input placeholder="Setor..." style={{padding: '0.3rem', fontSize: '0.8rem'}} value={editingRole.sector || ''} onChange={e => setEditingRole({...editingRole, sector: e.target.value})} />
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                      <strong style={{ fontSize: '0.85rem' }}>{currentRoleObj.name}</strong>
                      <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{currentRoleObj.sector || 'Sem setor vinculado'}</span>
                    </div>
                  )}
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    {editingRole ? (<><button onClick={handleUpdateRole} style={{ color: 'var(--success-color)' }}><Check size={16} /></button><button onClick={() => setEditingRole(null)}><X size={16} /></button></>) : (<><button onClick={() => setEditingRole({id: currentRoleObj.id, name: currentRoleObj.name, sector: currentRoleObj.sector || ''})}><Edit2 size={14} /></button><button onClick={() => handleDeleteRole(selectedRoleId)} style={{ color: 'var(--danger-color)' }}><Trash2 size={14} /></button></>)}
                  </div>
                </div>
              </div>
            )}
            
            <form onSubmit={handleAddRole} style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <input type="text" placeholder="Nome da Função..." style={{ padding: '0.5rem', fontSize: '0.85rem' }} value={newRoleForm.name} onChange={e => setNewRoleForm({...newRoleForm, name: e.target.value})} />
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <input type="text" placeholder="Setor (Opcional)..." style={{ flex: 1, padding: '0.5rem', fontSize: '0.85rem' }} value={newRoleForm.sector} onChange={e => setNewRoleForm({...newRoleForm, sector: e.target.value})} />
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ backgroundColor: 'var(--surface-color)', padding: '2rem', borderRadius: 'var(--radius-lg)', width: '100%', maxWidth: '550px' }}>
            ...
            <form onSubmit={handleSaveUserAccess} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '700', marginBottom: '0.5rem' }}>Restrição por Unidades</label>
                <div style={{ border: '1px solid var(--border-color)', borderRadius: '8px', padding: '1rem', maxHeight: '200px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.6rem', backgroundColor: 'var(--bg-color)' }}>
                  {units.map(u => (
                    <label key={u.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.9rem', cursor: 'pointer', padding: '0.2rem 0' }}>
                      <input type="checkbox" checked={Array.isArray(editingUser.unit_ids) && editingUser.unit_ids.includes(u.id)} onChange={() => toggleUserUnit(u.id)} style={{ width: '18px', height: '18px', accentColor: 'var(--saritur-orange)', cursor: 'pointer' }} /> 
                      {u.name}
                    </label>
                  ))}
                </div>
              ...
              {editingUser?.role !== 'ADMIN' && (
                <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '1rem' }}>
                  <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '700', marginBottom: '0.5rem', color: 'var(--saritur-orange)' }}>Período de Férias</label>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                    <div><label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '600' }}>Início</label><input type="date" style={{ width: '100%', padding: '0.6rem', borderRadius: '4px', border: '1px solid var(--border-color)' }} value={editingUser.vacation_start ? editingUser.vacation_start.split('T')[0] : ''} onChange={e => setEditingUser({...editingUser, vacation_start: e.target.value})} /></div>
                    <div><label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '600' }}>Fim</label><input type="date" style={{ width: '100%', padding: '0.6rem', borderRadius: '4px', border: '1px solid var(--border-color)' }} value={editingUser.vacation_end ? editingUser.vacation_end.split('T')[0] : ''} onChange={e => setEditingUser({...editingUser, vacation_end: e.target.value})} /></div>
                  </div>
          ...
          <div style={{ backgroundColor: 'var(--surface-color)', padding: '2rem', borderRadius: 'var(--radius-lg)', width: '100%', maxWidth: '700px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={handleSaveWidget} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div>
                ...
                <input required type="text" style={{ width: '100%', padding: '0.6rem', border: '1px solid var(--border-color)', borderRadius: '4px' }} placeholder="Ex: Lead Time de Admissão" value={widgetForm.title} onChange={e => setWidgetForm({...widgetForm, title: e.target.value})} />
              </div>
