...
export default function ConfiguracoesPage() {
  ...
  // --- ESTADOS DE SELEÇÃO E EDIÇÃO (DROPDOWNS) ---
  ...
  // --- ESTADOS DO CONSTRUTOR DE DASHBOARD ---
  const [editingWidget, setEditingWidget] = useState(null);
  const [isWidgetModalOpen, setIsWidgetModalOpen] = useState(false);
  const [widgetForm, setWidgetForm] = useState({ 
    title: '', chart_type: 'kpi', metric_type: 'count', 
    status_filter: 'Todos', color: '#F37137', roles_visible: ['ADMIN'],
    advanced_config: {}
  });
  ...
  const menusAcessiveis = [
    ...
    { path: '/pre-admissao', label: 'Pipeline' }, 
  ...
  async function fetchAllData() {
    try {
      const [unitsRes, rolesRes, reasonsRes, usersRes, permsRes, templatesRes, widgetsRes] = await Promise.all([
        ...
        api.messageTemplates.list().catch(() => []),
        api.dashboardWidgets.list().catch(() => []) 
      ]);
      setUnits(unitsRes || []); 
      setRoles(rolesRes || []); 
      setCancellationReasons(reasonsRes || []);
  ...
  async function handleSaveWidget(e) {
    ...
    try {
      ...
      alert('Erro ao salvar Indicador: ' + err.message); 
    }
  }
  ...
  // --- FUNÇÕES DE MATRIZ DE PERMISSÕES ---
  ...
  // --- FUNÇÕES DE MENSAGENS / TEMPLATES ---
  ...
  // --- FUNÇÕES DE DROPDOWNS E USUÁRIOS ---
  ...
  if (isAdmin === false) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh' }}>
        ...
        <p style={{ color: 'var(--text-muted)' }}>Você não tem permissão para acessar o painel de governança.</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem', paddingBottom: '3rem' }}>
      ...
      {/* BLOCO 1: CONSTRUTOR DE DASHBOARD */}
      <div className="glass-panel" style={{ padding: '2rem', backgroundColor: 'var(--surface-color)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          ...
          <button onClick={() => { 
            setEditingWidget(null); 
            setWidgetForm({ title: '', chart_type: 'kpi', metric_type: 'count', status_filter: 'Todos', color: '#F37137', roles_visible: ['ADMIN'], advanced_config: {} }); 
            setIsWidgetModalOpen(true); 
          }} className="btn-primary" style={{ padding: '0.5rem 1rem', fontSize: '0.85rem' }}>
            <Plus size={16} style={{marginRight: '4px'}}/> Criar Indicador
          </button>
        </div>
      ...
      {/* BLOCO 2: DROPDOWNS E CADASTROS BÁSICOS */}
      <div className="glass-panel" style={{ padding: '2rem', backgroundColor: 'var(--surface-color)' }}>
        <h2 style={{ fontSize: '1.15rem', fontWeight: 'bold', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <Settings2 size={20} color="var(--saritur-orange)" /> Tabelas Estruturais (Dropdowns)
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '2rem' }}>
          ...
          <div style={{ borderRight: '1px solid var(--border-color)', paddingRight: '1rem' }}>
            ...
            <select style={{ width: '100%', marginBottom: '1rem', padding: '0.5rem' }} value={selectedUnitId} onChange={e => { setSelectedUnitId(e.target.value); setEditingUnit(null); }}>
            ...
            {selectedUnitId && currentUnitObj && (
              <div style={{ backgroundColor: 'var(--bg-color)', padding: '0.75rem', borderRadius: 'var(--radius-md)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                {editingUnit ? <input style={{width: '70%', padding: '0.2rem'}} value={editingUnit.name} onChange={e => setEditingUnit({...editingUnit, name: e.target.value})} /> : <strong style={{ fontSize: '0.85rem' }}>{currentUnitObj.name}</strong>}
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  {editingUnit ? (
                    <><button onClick={handleUpdateUnit} style={{ color: 'var(--success-color)' }}><Check size={16} /></button><button onClick={() => setEditingUnit(null)}><X size={16} /></button></>
                  ) : (
            ...
            <form onSubmit={handleAddUnit} style={{ display: 'flex', gap: '0.5rem' }}>
              <input type="text" placeholder="Nova Unidade..." style={{ flex: 1, padding: '0.5rem' }} value={newUnit} onChange={e => setNewUnit(e.target.value)} />
          ...
          <div style={{ borderRight: '1px solid var(--border-color)', paddingRight: '1rem' }}>
            ...
            {selectedRoleId && currentRoleObj && (
              <div style={{ backgroundColor: 'var(--bg-color)', padding: '0.75rem', borderRadius: 'var(--radius-md)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                {editingRole ? <input style={{width: '70%', padding: '0.2rem'}} value={editingRole.name} onChange={e => setEditingRole({...editingRole, name: e.target.value})} /> : <strong style={{ fontSize: '0.85rem' }}>{currentRoleObj.name}</strong>}
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  {editingRole ? (
                    <><button onClick={handleUpdateRole} style={{ color: 'var(--success-color)' }}><Check size={16} /></button><button onClick={() => setEditingRole(null)}><X size={16} /></button></>
                  ) : (
            ...
            <form onSubmit={handleAddRole} style={{ display: 'flex', gap: '0.5rem' }}>
              <input type="text" placeholder="Nova Função..." style={{ flex: 1, padding: '0.5rem' }} value={newRole} onChange={e => setNewRole(e.target.value)} />
              <button type="submit" className="btn-primary" style={{ padding: '0.5rem' }}><Plus size={16} /></button>
            </form>
          </div>
          ...
          <div>
            ...
            <select style={{ width: '100%', marginBottom: '1rem', padding: '0.5rem' }} value={selectedReasonId} onChange={e => { setSelectedReasonId(e.target.value); setEditingReason(null); }}>
            ...
            {selectedReasonId && currentReasonObj && (
              <div style={{ backgroundColor: 'var(--bg-color)', padding: '0.75rem', borderRadius: 'var(--radius-md)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                {editingReason ? <input style={{width: '70%', padding: '0.2rem'}} value={editingReason.name} onChange={e => setEditingReason({...editingReason, name: e.target.value})} /> : <strong style={{ fontSize: '0.85rem' }}>{currentReasonObj.name}</strong>}
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  {editingReason ? (
                    <><button onClick={handleUpdateReason} style={{ color: 'var(--success-color)' }}><Check size={16} /></button><button onClick={() => setEditingReason(null)}><X size={16} /></button></>
                  ) : (
            ...
            <form onSubmit={handleAddReason} style={{ display: 'flex', gap: '0.5rem' }}>
              <input type="text" placeholder="Novo Motivo..." style={{ flex: 1, padding: '0.5rem' }} value={newReason} onChange={e => setNewReason(e.target.value)} />
              <button type="submit" className="btn-primary" style={{ padding: '0.5rem' }}><Plus size={16} /></button>
            </form>
          </div>
        </div>
      ...
      {/* BLOCO 3: GESTÃO DE USUÁRIOS */}
      <div className="glass-panel" style={{ padding: '2rem', backgroundColor: 'var(--surface-color)' }}>
        <h2 style={{ fontSize: '1.15rem', fontWeight: 'bold', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <Users size={20} color="var(--saritur-orange)" /> Controle de Acesso e Perfis
        </h2>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid var(--border-color)', textAlign: 'left', color: 'var(--text-muted)' }}>
                ...
                <th style={{ padding: '0.75rem' }}>Unidade</th>
              ...
              {usersList.map(user => (
                <tr key={user.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <td style={{ padding: '0.75rem', textAlign: 'right' }}>
                    {user.status === 'Pendente' && (
                      <button onClick={() => handleApproveUser(user.id)} className="btn-secondary" style={{ color: 'var(--success-color)', padding: '0.3rem', marginRight: '0.5rem' }}>
                        <Check size={14} />
                      </button>
                    )}
                    <button onClick={() => handleDeleteUser(user.id)} className="btn-secondary" style={{ color: 'var(--danger-color)', padding: '0.3rem' }}>
                      <Trash2 size={14} />
                    </button>
                  </td>
                </tr>
      ...
      {/* BLOCO 4: MATRIZ DE PERMISSÕES (O QUE SUMIU ANTES AGORA ESTÁ AQUI) */}
      <div className="glass-panel" style={{ padding: '2rem', backgroundColor: 'var(--surface-color)' }}>
        <h2 style={{ fontSize: '1.15rem', fontWeight: 'bold', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <ShieldAlert size={20} color="var(--saritur-orange)" /> Matriz de Permissões
        </h2>
        <div style={{ overflowX: 'auto' }}>
      ...
      {/* BLOCO 5: TEMPLATES DE WHATSAPP */}
      <div className="glass-panel" style={{ padding: '2rem', backgroundColor: 'var(--surface-color)' }}>
        <h2 style={{ fontSize: '1.15rem', fontWeight: 'bold', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <MessageSquareText size={20} color="var(--saritur-orange)" /> Modelos de Mensagens (WhatsApp)
        </h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          {templates.map(template => (
            <div key={template.id} style={{ padding: '1.25rem', backgroundColor: 'var(--bg-color)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)' }}>
              ...
              {editingTemplateId === template.id ? (
                <textarea style={{ width: '100%', minHeight: '80px', padding: '0.75rem' }} value={editingTemplateContent} onChange={e => setEditingTemplateContent(e.target.value)} />
              ) : (
      ...
      {/* BLOCO 6: METAS DO DASHBOARD */}
      <div className="glass-panel" style={{ padding: '2rem', backgroundColor: 'var(--surface-color)' }}>
        <h2 style={{ fontSize: '1.15rem', fontWeight: 'bold', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <BarChart3 size={20} color="var(--saritur-orange)" /> Metas Globais do Dashboard
        </h2>
        <form onSubmit={handleSaveTargets} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '1.5rem', alignItems: 'end' }}>
          <div>
            <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '600', marginBottom: '0.5rem' }}>Meta Leadtime (Dias)</label>
            <input type="number" style={{padding: '0.5rem', width: '100%', borderRadius: '4px', border: '1px solid var(--border-color)'}} placeholder="Ex: 15" value={dashTargets.targetLeadtime} onChange={e => setDashTargets({...dashTargets, targetLeadtime: e.target.value})} />
          </div>
          <div>
            <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '600', marginBottom: '0.5rem' }}>Taxa de Aprovação (%)</label>
            <input type="number" style={{padding: '0.5rem', width: '100%', borderRadius: '4px', border: '1px solid var(--border-color)'}} placeholder="Ex: 60" value={dashTargets.targetApprovalRate} onChange={e => setDashTargets({...dashTargets, targetApprovalRate: e.target.value})} />
          </div>
          <button type="submit" className="btn-primary" style={{ padding: '0.65rem 1.5rem' }}><Save size={16} /> Salvar Parâmetros</button>
        </form>
      </div>

      {/* --- MODAL DE CONSTRUÇÃO DE INDICADOR (WIDGET) --- */}
      {isWidgetModalOpen && (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                ...
                <div>
                  ...
                  <select style={{ width: '100%', padding: '0.6rem', border: '1px solid var(--border-color)', borderRadius: '4px' }} value={widgetForm.metric_type} onChange={e => setWidgetForm({...widgetForm, metric_type: e.target.value, advanced_config: { ...widgetForm.advanced_config, formula: e.target.value }})}>
                    ...
                    <option value="date_diff">Operação Matemática (Data Fim - Data Início)</option>
                  </select>
                </div>
              </div>
